...
from sklearn.ensemble._forest import BaseForest # type: ignore
from sklearn.neighbors import NearestNeighbors # type: ignore
from sklearn.preprocessing import normalize # type: ignore
from sklearn.utils import check_random_state, check_array, check_X_y # type: ignore
from sklearn.tree import BaseDecisionTree # type: ignore
...
class SMOTEBoost(AdaBoostClassifier):
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, sample_weight_list: Optional[np.ndarray] = None, minority_target: Optional[int] = None) -> object:
        ...
        accept_sparse: Union[str, List[str]]
        ...
        if sample_weight_list is None:
            ...
            sample_weight = check_array(sample_weight_list, ensure_2d=False)
        ...
        if minority_target is None:
            ...
            stats_c_: Counter[int] = Counter(y)
            key: Callable[..., int] = stats_c_.get
            maj_c_ = max(stats_c_, key=key)
            min_c_ = min(stats_c_, key=key)
